...
=> Laplacian기법의 정확도가 가장 높아 이 전처리 기법을 사용한 모델을 서버(백앤드)에 이식하여 사용했다.
...
#### Dogaccount app
 애완견의 프로필의 값을 DB에 저장하고 애완견의 비문등록 및 비문검사를 진행한다. DB에 저장을 완료한 후, 사용자가 프론트에서 애완견 프로필 정보 및 id(pk)값을 보내면, 서버에서 받아, 해당 id 값에 맡는 애완견의 DB 정보를 가져와서 조회 및 수정을 진행한다.
  사용자가 비문 등록 및 인식할 때 애완견의 코 사진을 따로 DB에 저장한 후, Foreign key 를 활용하여 강아지의 프로필 정보의 DB와 연결시킨다. 

 사용자가 비문 인식을 통해 강아지의 프로필을 조회할 때, 강아지의 사진을 찍어 보내면 애완견의 비문과 DB에 저장되어있는 비문을 이진탐색 알고리즘을 활용하여 비문 인식 모델에 input으로 넣으면, 예측값을 client로 전송한다. 만약 비교 비문이 유사할 경우, 해당 비문의 강아지의 프로필 정보를 조회할 수 있도록 했다.
